...
public class Face
{
    ...
    public static bool RayHitTriangle(Vector3 origin, Vector3 direction, Vector3 v0, Vector3 v1, Vector3 v2,
    ...
    public override string ToString()
    {
        // return "F(" + a +"," + b + "," + c + "," + d + ")";
        return IsTriangle
            ? $"F(a={A},b={B},c={C},normal={Normal},tags={Tags})"
            : $"F(a={A},b={B},c={C},d={D},normal={Normal},tags={Tags})";
    }
    ...
    public Face SetUvProjected(float uvScale)
    {
        ...
        if (!IsTriangle)
        {
            UvD = new Vector2((dlr * D.Z + dfb * D.X + dud * D.X) * uvScale,
                (dlr * D.Y + dfb * D.Y + dud * D.Z) * uvScale);
        }
    ...
    private static Vector2 UvCylinderProjection(Vector3 vertex, Vector3 center, Vector3 direction, float uOffset,
        float uvScale)
    {
