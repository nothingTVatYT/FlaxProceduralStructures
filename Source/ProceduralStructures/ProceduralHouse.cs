...
public class ProceduralHouse
{
    ...
    {
        public QueuedMakeHole(Vector3 origin, Vector3 direction, Vector3 up, float width, float height,
            Material material, float maxDistance = 0, float uvScale = 0.01f)
        {
            this.origin = origin;
    ...
    */
    ...
    public Building RebuildHouseWithInterior(HouseDefinition house, Actor target, int lod)
    {
        ...
        foreach (var bs in house.layers)
        {
            ...
            if (bs.stairs != null)
            {
                foreach (var stairs in bs.stairs)
                {
                    ...
                    var ou = Vector3.Backward * Mathf.Cos(stairs.descentAngle * Mathf.DegreesToRadians) *
                             stairs.stepDepth;
                    ...
                    stairsBlock.Position = center + new Vector3(stairs.offset, stairs.baseHeight,
                        -stairs.baseLength / 2 - zOffset);
                    if (stairs.inside)
                    {
                        ...
                        delayedBoring.Add(new QueuedMakeHole(holePosition, Vector3.Up, Vector3.Backward,
                            stairsBounds.Size.X - 10f, stairsBounds.Size.Z - 10f, bs.material, maxDistance));
                    }
        ...
        if (house.roofHeight > 0)
        {
            ...
            {
                var innerFrontFace = frontFace.DeepCopy().MoveFaceBy(Vector3.Forward * lastWallThickness)
                    .InvertNormals();
                var innerBackFace = backface.DeepCopy().MoveFaceBy(Vector3.Backward * lastWallThickness)
                    .InvertNormals();
                ...
                var innerLeftTopWall = new Face(a, b, b + Vector3.Right * lastWallThickness,
                    a + Vector3.Right * lastWallThickness);
                innerLeftTopWall.SetUvFront(lastWallThickness, length);
                var innerRightTopWall = new Face(d + Vector3.Left * lastWallThickness,
                    c + Vector3.Left * lastWallThickness, c, d);
