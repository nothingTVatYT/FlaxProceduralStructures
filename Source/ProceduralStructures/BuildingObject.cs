...
public class BuildingObject
{
    ...
    public BuildingObject SetUVBoxProjection(float uvScale)
    {
        foreach (var face in Faces)
        {
            ...
            face.UvA = new Vector2((dlr * face.A.Z + dfb * face.A.X + dud * face.A.X) * uvScale,
                (dlr * face.A.Y + dfb * face.A.Y + dud * face.A.Z) * uvScale);
            face.UvB = new Vector2((dlr * face.B.Z + dfb * face.B.X + dud * face.B.X) * uvScale,
                (dlr * face.B.Y + dfb * face.B.Y + dud * face.B.Z) * uvScale);
            face.UvC = new Vector2((dlr * face.C.Z + dfb * face.C.X + dud * face.C.X) * uvScale,
                (dlr * face.C.Y + dfb * face.C.Y + dud * face.C.Z) * uvScale);
            if (!face.IsTriangle)
            {
                face.UvD = new Vector2((dlr * face.D.Z + dfb * face.D.X + dud * face.D.X) * uvScale,
                    (dlr * face.D.Y + dfb * face.D.Y + dud * face.D.Z) * uvScale);
            }
        }
    ...
    public BuildingObject MakeHole(Vector3 origin, Vector3 direction, Vector3 up, float width, float height,
        Material material = null, float maxDistance = 0f, float uvScale = 0.01f)
    {
        ...
        for (var cut = 0; cut < 4; cut++)
        {
            ...
            foreach (var face in affectedFaces)
            {
                if (face.RayHit(origin, direction, false, out fromBack, out intersection))
                {
                    ...
                    if (Mathf.Sign(dA) != Mathf.Sign(dB) || Mathf.Sign(dB) != Mathf.Sign(dC) ||
                        Mathf.Sign(dC) != Mathf.Sign(dD))
                    {
                        ...
                        if (thisCutFace != null)
                        {
                            if (previousCutFace != null)
                            {
                                var bridged = Builder.BridgeEdgeLoops(
                                    new List<Vector3>
                                        { previousCutFace.A, previousCutFace.B, previousCutFace.C, previousCutFace.D },
    ...
    public BuildingObject CutFront(Rectangle dim, bool indent, float uvScale)
    {
        ...
        foreach (var face in Faces)
        {
            ...
            if (cutoutFace.A.X > face.A.X && cutoutFace.A.Y >= face.A.Y && cutoutFace.C.X < face.C.X &&
                cutoutFace.C.Y <= face.C.Y)
            {
