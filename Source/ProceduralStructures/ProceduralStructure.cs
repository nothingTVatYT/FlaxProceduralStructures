...
*/
    ...
    public void RebuildCave(CaveDefinition cave, WayPointList tunnel, Actor target)
    {
        ...
        foreach (var tangent in cave.GetTangents(tunnel))
        {
            ...
            if (previousEdgeLoop != null)
            {
                ...
                var cylinderCenter =
                    cavemesh.GetCenter(
                        currentEdgeLoop); //(cavemesh.GetCenter(currentEdgeLoop) + cavemesh.GetCenter(previousEdgeLoop))/2;
                cavemesh.SetUVTunnelProjection(generatedTriangles, cylinderCenter, planeNormal, uOffset, cave.UScale,
                    cave.VScale);
    ...
    private List<Vector3> ConstructOShape(float width, float height)
    {
        ...
        for (var i = 0; i < 12; i++)
        {
            result.Add(new Vector3(width / 2 * Mathf.Sin(Mathf.DegreesToRadians * i * 30),
                height / 2 + height / 2 * Mathf.Cos(Mathf.DegreesToRadians * i * 30), 0));
        }
    ...
    public void ConstructFrameHouse(FrameHouse house, Actor target)
    {
        ...
        foreach (var edge in frame.Edges)
        {
            ...
            {
                frameConstruction.AddObject(Beam(frame.Points[edge.A], frame.Points[edge.B], house.BeamThickness,
                    house.UvScale));
            }
        }
        ...
        Debug.Log(sw.Stop() + ", vertices: " + frameConstruction.VerticesCount + ", triangles: " +
                  frameConstruction.TrianglesCount);
