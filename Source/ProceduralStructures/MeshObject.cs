...
public class MeshObject
{
    ...
    private static IEnumerable<List<Vertex>> FollowLoop(Stack<Vertex> l, Vertex head, HashSet<Vertex> set,
        HashSet<TEdge> visitedEdges, int maxLength, IReadOnlySet<EdgeLoop> foundEdgeLoops)
    {
        if (l == null)
        {
            l = new Stack<Vertex>();
    ...
    private void CheckOnOverlaps(IList<TEdge> edges)
    {
        ...
        do
        {
            ...
            for (var i = 0; i < edges.Count - 1; i++)
            {
                ...
                {
                    var edge2 = edges[j];
                    if (GeometryTools.EdgeEdgeIntersectIgnoreEnds(edge1.A.Pos, edge1.B.Pos, edge2.A.Pos, edge2.B.Pos,
    ...
    public bool RayHitTriangle(Vector3 origin, Vector3 direction, bool ignoreFromBack, out Triangle triangleHit,
        out bool fromBack, out Vector3 intersection)
    {
        fromBack = false;
    ...
    public int GetTriangleHit(Vector3 origin, Vector3 direction)
    {
        foreach (var triangle in Triangles)
        {
            ...
            if (Face.RayHitTriangle(origin, direction, triangle.V0.Pos, triangle.V1.Pos, triangle.V2.Pos,
                    out intersection))
            {
    ...
    public void SetUVCylinderProjection(IEnumerable<int> triangleIndices, Vector3 center, Vector3 direction,
    ...
    public void SetUVTunnelProjection(IEnumerable<int> triangleIndices, Vector3 center, Vector3 direction,
        float uOffset, float uScale, float vScale)
    {
        foreach (var ti in triangleIndices)
        {
    ...
    public void SetUVBoxProjection(float uvScale)
    {
        ...
        foreach (var face in Triangles)
        {
            ...
            face.Uv0 = new Vector2((dlr * face.V0.Pos.Z + dfb * face.V0.Pos.X + dud * face.V0.Pos.X) * uvScale,
                (dlr * face.V0.Pos.Y + dfb * face.V0.Pos.Y + dud * face.V0.Pos.Z) * uvScale);
            face.Uv1 = new Vector2((dlr * face.V1.Pos.Z + dfb * face.V1.Pos.X + dud * face.V1.Pos.X) * uvScale,
                (dlr * face.V1.Pos.Y + dfb * face.V1.Pos.Y + dud * face.V1.Pos.Z) * uvScale);
            face.Uv2 = new Vector2((dlr * face.V2.Pos.Z + dfb * face.V2.Pos.X + dud * face.V2.Pos.X) * uvScale,
                (dlr * face.V2.Pos.Y + dfb * face.V2.Pos.Y + dud * face.V2.Pos.Z) * uvScale);
        }
    }
    ...
    public List<Vertex> FindBoundaryAround(List<Vertex> surounding)
    {
        ...
        if (edges.Count == 0)
        {
            Debug.LogWarning("expected non-manifold edges from: " + string.Join(",", attachedTriangles) +
                             " using " + string.Join(",", surounding));
            return result;
        }
    ...
    public List<Vertex> SortConnectedVertices(ICollection<Vertex> l)
    {
        ...
        while (loop.Count > 0)
        {
            foreach (var triangle in start.Triangles)
            {
                ...
                {
                    Debug.LogWarning("A triangle is linked in " + start + " which is not part of this object: " +
                                     triangle);
                    continue;
                }
            ...
            {
                Debug.LogWarning("The list is not connected. " + loop.Count + " vertices are left: " +
                                 string.Join(" ", loop));
                break;
            }
        }
